...
my $R1 = `cd $data && find -type f -iname "*1*gz" -printf "%f"`;
my $R2 = `cd $data && find -type f -iname "*2*gz" -printf "%f"`;
chomp $R1;
chomp $R2;
...
elsif($protocol eq "10Xv2"){
		$protocol        = "CCCCCCCCCCCCCCCCNNNNNNNNNN";
}
    elsif($protocol eq "10Xv3"){
           $protocol	 = "CCCCCCCCCCCCCCCCNNNNNNNNNNNN";
}
